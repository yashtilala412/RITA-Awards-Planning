...
var cowner = components.form1.data.inp_cowner.value;
...
var comob = components.form1.data.inp_ownermobile.value;
var phoneno = /^\d{10}$/;
if(!components.form1.data.inp_ownermobile.value.match(phoneno)){
  	await actions.setVariable('comob',true);
}else{
	await actions.unSetVariable('comob');
  i++;
}
var caddress = components.form1.data.inp_caddress.value;
...
if (!components.form1.data.inp_cemail.value.match(validRegex)) {
  		await actions.setVariable('cemail',true);
}else{
	await actions.unSetVariable('cemail');
  i++;
}
var cweb = components.form1.data.inp_cwebsite.value;
...
var ctype = components.form1.data.drpdown_ctype.value;
...
var caward  =  components.form1.data.multi_caward.values;
//alert(caward.length)
if(caward.length>5)
{
...
var cfoundingyear= components.form1.data.inp_fyear.value;
var regexfor = /^([0-9]{4})$/;
if(!regexfor.test(components.form1.data.inp_fyear.value)){
	await actions.setVariable('cfoundingyear',true);
}else{
	await actions.unSetVariable('cfoundingyear');
  i++;
}
var cstrength = components.form1.data.csstrength.value;
var regexforStrength =  /^(\s*(?:\d{1,5}(?:,\d{5})?|null)(?:\s*-\s*(?:\d{1,5}(?:,\d{5})?|null))?\s*)(,\s*(?:\d{1,5}(?:,\d{5})?|null)(?:\s*-\s*(?:\d{1,5}(?:,\d{5})?|null))?\s*)*$/;
if(!regexforStrength.test(components.form1.data.csstrength.value) || cstrength.length<1){
...
var cproof = components.form1.data.drpdwn_cproof.value;
...
var fielUp = components.filepicker4.file.length;
if(isuploaded==1){
	i++;
}else if(fielUp>0){
