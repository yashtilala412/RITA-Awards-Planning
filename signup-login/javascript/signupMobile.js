function generateRandomString(length) {
  ...
  var pass = components.form2.data.inp_password_mobile.value.replace(/'/g, '\\');
  var emai = components.form2.data.inp_email_mobile.value;
  var passlen = components.form2.data.inp_password_mobile.value.length;
  var confpass = components.form2.data.inp_confpass_mobile.value.replace(/'/g, '\\');
  ...
  async function run1() {
    ...
    if(data1.data.length<=0){
      ...
      components.form2.resetForm();
          await components.text39.setText('Thank you for nomination please check your email for verification <br> once verification is successfully completed we will proceed to the next step <br> for more details feel free to contact us') ;   
  ...
  if(pass===confpass){
    ...
    if (!components.form2.data.inp_email_mobile.value.match(validRegex)) {
